...
# train_loss_log_path = ".\\data\\train_loss_log_No_GRU.txt"
...
def run():
    ...
    full_saver.restore(sess, ".\\log_512_3\\resnet_convgru_7499_train_loss=0.0093_test_loss=0.0140.ckpt")
    ...
    def generate_test_batch(test_batch_size, annoList, ind):
        ...
        batch_inds = range(ind, ind+test_batch_size)
    ...
    for i in range(0, num_test_batches, batch_size):
        last_time = time.time()

        test_input_batch, test_output_batch = generate_test_batch(batch_size, test_anno, i)
        test_batch_feed = {input_placeholder: test_input_batch,
                      output_placeholder: test_output_batch}

        test_batch_predictions = sess.run(predictions, feed_dict=test_batch_feed)

        io.savemat('.\\predict\\' + str(i) + '_test_batch_predictions.mat', {'test_batch_predictions': test_batch_predictions})
